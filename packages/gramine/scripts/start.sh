...
DOCKER_COMMAND="sudo docker run -d -p 3000:3000 --env-file .env --rm --device /dev/sgx_enclave --device /dev/sgx_provision -v $(pwd)/dist:/dist -it gramineproject/gramine bash -c 'cd ./dist; mkdir ./data; chmod +x -R .; ./gramine-sgx bazk ./app/index.js'"
